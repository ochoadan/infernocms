...
export type MigrationOp =
  | { type: 'CreateTable'; table: string; destructive: false }
  | { type: 'AddColumn'; table: string; column: string; definition: string; destructive: false }
  | { type: 'DropColumn'; table: string; column: string; destructive: true }
  | { type: 'AlterColumn'; table: string; column: string; sql: string; description: string; destructive: boolean }
  | { type: 'CreateIndex'; table: string; sql: string; destructive: false }
...
export function diffSchema(
  ...
  knownCmsTables?: Set<string>
): MigrationOp[] {
  const ops: MigrationOp[] = [];
  const desiredTables = new Set<string>();
  ...
  for (const [tableName, collection] of Object.entries(config.collections)) {
    ...
    const desiredColumns = new Set<string>();
    ...
    for (const [fieldName, fieldConfig] of Object.entries(collection.fields)) {
      ...
      if (!colInfo) {
        ...
        if (fieldConfig.type === 'slug') parts.push('UNIQUE');
  ...
  for (const [tableName] of actual) {
    if (tableName.startsWith('_infernocms_')) continue;
    if (desiredTables.has(tableName)) continue;
    // Only drop if we know this table was previously created by InfernoCMS
    if (knownCmsTables && !knownCmsTables.has(tableName)) continue;
