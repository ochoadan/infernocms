import type { NormalizedConfig, NormalizedFieldConfig, FieldType } from '../config/types.js';
import { getDb } from './connection.js';

function fieldTypeToPostgres(field: NormalizedFieldConfig): string {
...
export async function syncTables(config: NormalizedConfig): Promise<void> {
  const db = getDb();
      ...
      if (tableInfo.exists) {
        ...
        for (const [fieldName, fieldConfig] of Object.entries(collection.fields)) {
          ...
          if (!existingColumns.has(fieldName)) {
            ...
            await db.exec(
              `ALTER TABLE "${name}" ADD COLUMN "${fieldName}" ${pgType}${uniqueClause}${defaultClause};`
            );
          }
        }
