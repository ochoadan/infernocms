...
`0.1.0` shipped 2026-05-02. The schema engine, REST API, hooks, webhooks, access control, PGlite/Postgres drivers, local + S3 storage, and type generation are stable. The admin UI is preview-only — functional in the monorepo, not yet packaged for external `npm install`.
...
| 3 | Admin UI rebuild on shadcn preset `b1FktqLQI` | Not started | TBD |
| 4 | User-facing docs overhaul (LLM-consumable first) | Not started | TBD |
| 5 | Inferno Cloud `next-auth` integration (separate repo) | Not started | TBD |
| 6 | End-to-end audit + cleanup of orphaned/misaligned code | Not started | TBD |

## What's next (after sub-projects 1–6)

- **Admin packaging** — publish `@infernocms/admin` as a separate npm package or bundle a built admin into core, so external installs get the full UI without needing the monorepo. Currently blocked behind the admin rebuild (sub-project 3).
...
| Version history (rollbacks) | Useful but adds significant DB and admin UI surface; revisit after sub-project 6. |
| Real-time collaboration (Google-Docs-style) | Out of scope. Token-based auth + conflict-on-save is the model. |
| Multi-tenancy in a single instance | Not for InfernoCMS itself — multi-tenancy is provided by Inferno Cloud, which provisions a separate CMS instance per project. |
